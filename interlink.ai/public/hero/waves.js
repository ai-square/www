...
(function () {
  ...
  // Check if an element with the '.hero-background-animation' class exists
  if (!document.querySelector('.hero-background-animation')) {
      return;
  }
...
(function () {
    ...
    this.Waves = function (holder, options) {
      ...
      // Check if a canvas already exists in the holder
      var existingCanvas = Waves.holder.querySelector('canvas');
      if (!existingCanvas) {
          Waves.canvas = document.createElement('canvas');
          Waves.ctx = Waves.canvas.getContext('2d');
          Waves.holder.appendChild(Waves.canvas);
      } else {
          Waves.canvas = existingCanvas;
          Waves.ctx = existingCanvas.getContext('2d');
      }
    ...
    Waves.prototype.resize = function () {
      ...
      // Get the device pixel ratio, capped at a maximum value of 2 for performance reasons
      Waves.scale = Math.min(window.devicePixelRatio || 1, .9);
      ...
      // Waves.radius /= 2; // REMOVE FOR FULLSCREEN
  };
